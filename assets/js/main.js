(function($) {
		...
		$('.gallery')
			.on('click', 'a', function(event) {
				var $a = $(this),
					...
					$innerPanel = $modal.find('.gallery-panel'),
					href = $a.attr('href'),
				...
				// Set src.
					...
					if (info == undefined) {
						...
						var jsonInfo = JSON.parse(decodeURI(info))
						$innerPanel.empty().append('<h2 class="panel-name">' + jsonInfo.name + '</h2>\
											<p class="panel-motto">' + jsonInfo.mote + '</p>\
											<p class="panel-teacher">师承：' + jsonInfo.teacher + '</p>').show()
					}
				// Set visible.
			...
			.prepend('<div class="modal" tabIndex="-1"><div class="inner"><img src="" /><div class="gallery-panel"></div></div></div>')
				.find('img')
	...
	// Custom Data Load
		$.get("staffs.json", function(res){
			...
			$(".avatars img.lazyload").lazyload()
		})
		$.get("slides.json", function(res){
			$.each(res, function(index, value){
